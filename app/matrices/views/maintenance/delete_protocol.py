    ...
    if request.user.is_superuser:
        ...
        if protocol:
            ...
            if exists_command_for_protocol(protocol):
                ...
                messages.error(request, 'CPW_WEB:0490 Protocol ' + protocol.name +
                               ' NOT Deleted - API Commands still exist!')

            else:

                if exists_blog_command_for_protocol(protocol):

                    messages.error(request, 'CPW_WEB:0500 Protocol ' + protocol.name +
                                   ' NOT Deleted - Blog Commands still exist!')
