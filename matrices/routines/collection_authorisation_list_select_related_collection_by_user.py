...
"""
    Get All Collection Authorisations for All Collections for a particular User
"""
def collection_authorisation_list_select_related_collection_by_user(a_user):

    Collection = apps.get_model('matrices', 'Collection')

    CollectionAuthorisation = apps.get_model('matrices', 'CollectionAuthorisation')

    queryset = CollectionAuthorisation.objects.select_related('collection').filter(permitted=a_user)

    collection_authorisations = list()
    
    for collection_authorisation in queryset:
    
        sum_collections = Collection.objects.annotate(image_count=Count("images")).filter(id=collection_authorisation.collection.id)
        ...
        out_collection_authorisation = ({
            ...
            'authorisation_authority': collection_authorisation.collection_authority.name,
