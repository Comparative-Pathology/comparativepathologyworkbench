...
"""
    Get All Collections for All Users EXCEPT a particular User
"""
def collection_list_not_by_user(a_user):

    Collection = apps.get_model('matrices', 'Collection')

    queryset = Collection.objects.filter(~Q(owner=a_user)).order_by('id')
