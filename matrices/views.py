...
from matrices.models import authorisation_list_select_related_matrix_by_user
from matrices.models import authorisation_list_select_related_matrix
from matrices.models import matrix_list_by_user
from matrices.models import matrix_list_not_by_user
from matrices.models import matrix_list_all
from matrices.models import get_authority_for_matrix_and_user_and_requester
from matrices.models import credential_exists
from matrices.models import authorisation_exits_for_matrix_and_permitted
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        for cell in cell_list:
            ...
            if cell.image is not None:
                ...
                image = Image.objects.get(pk=cell.image.id)
...
def add_image(request, server_id, image_id, roi_id):
    ...
    if credential_exists(user) == True:
        ...
        if roi_id == '0':
            ...
            image = Image.create(image_id, name, server, viewer_url, birdseye_url, True, 0, request.user)
            ...
            for rois in json_rois:
                ...
                for shape in rois['shapes']:
                    ...
                    if shape['id'] == int(roi_id):
                        ...
                        image = Image.create(image_id, name, server, viewer_url, birdseye_url, True, roi_id, request.user)
...
def delete_image(request, image_id):
    ...
    if credential_exists(user) == True:
        ...
        return HttpResponseRedirect(reverse('list_image_cart', args=()))                
...
@login_required
def test_matrix(request, matrix_id):
    ...
    if credential_exists(user) == True:
        ...
        return render(request, 'matrices/test_matrix.html', data)
...
def view_matrix(request, matrix_id):
    ...
    if credential_exists(user) == True:
        ...
        link_post_url = blogLinkPost.protocol.name + '://' + serverWordpress.url_server + '/' + blogLinkPost.application + '/' + matrix.blogpost
...
def view_cell(request, matrix_id, cell_id):
    ...
    if credential_exists(user) == True:
        ...
        link_post_url = blogLinkPost.protocol.name + '://' + serverWordpress.url_server + '/' + blogLinkPost.application + '/' + cell.blogpost
    ...
    if credential_exists(user) == True:
        ...
        if authority.is_viewer == True or authority.is_none == True:
            ...
            if source_cell.has_image() == True:
                ...
                imageNew = Image.create(imageOld.identifier, imageOld.name, imageOld.server, imageOld.viewer_url, imageOld.birdseye_url, imageOld.active, imageOld.roi, imageOld.owner)
...
def delete_authorisation(request, authorisation_id):
    ...
    return HttpResponseRedirect(reverse('list_authorisation', args=()))                        
